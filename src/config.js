export const iconFilter = "M27 15h-4v4H0v4h23v4h4v-4h9v-4h-9zM13 4V0H9v4H0v4h9v4h4V8h23V4z";
export const iconHeart =
  "M27.57 3.241a6.885 6.885 0 0 1 5.032 2.118l.274.297a7.033 7.033 0 0 1 1.873 4.997 6.966 6.966 0 0 1-2.105 4.934L18.96 29.457 5.19 15.568a6.966 6.966 0 0 1-2.105-4.935 7.033 7.033 0 0 1 1.853-4.87l.294-.318a6.845 6.845 0 0 1 4.957-2.205 6.843 6.843 0 0 1 5.001 2.099l1.516 1.461 2.105 2.118 2.106-2.118 1.515-1.482a6.823 6.823 0 0 1 4.864-2.118l.273.042zM27.297.023a9.95 9.95 0 0 0-7.116 2.964l-1.495 1.461-1.516-1.46A9.991 9.991 0 0 0 9.89 0a9.995 9.995 0 0 0-7.183 3.22l-.274.296a10.245 10.245 0 0 0-2.42 7.136 10.131 10.131 0 0 0 2.947 7.179l16 16.115 16-16.136a10.131 10.131 0 0 0 2.947-7.158 10.24 10.24 0 0 0-2.695-7.03l-.273-.297A10.036 10.036 0 0 0 27.57.065l-.273-.042z";
export const iconGiftBox =
  "M24.96,6.197c0.607-0.653,0.986-1.541,0.986-2.521C25.946,1.632,24.313,0,22.335,0 c-1.502,0-4.964,2.246-6.348,4.264c-1.43-2.018-4.879-4.199-6.321-4.199c-1.973,0-3.612,1.633-3.612,3.677 c0,0.946,0.353,1.802,0.921,2.448H0v8.784h1.907V32h10.012h8.169H30.1V14.981H32V6.197H24.96z M22.335,1.365 c1.228,0,2.246,1.018,2.246,2.383c0,1.149-0.803,2.136-1.842,2.345h-5.524c-0.353-0.105-0.464-0.229-0.464-0.301 C16.751,4.63,20.833,1.365,22.335,1.365 M9.666,1.365c1.43,0,5.583,3.265,5.583,4.427c0,0.105-0.242,0.222-0.418,0.301H9.267 C8.229,5.884,7.425,4.897,7.425,3.749C7.419,2.449,8.444,1.365,9.666,1.365 M11.912,30.636H3.266V14.981h8.646V30.636z M11.912,13.688H1.365V7.563h10.554v6.126H11.912z M18.717,30.636H13.27V7.557h5.447V30.636z M28.729,30.636h-8.646V14.981h8.646 V30.636z M30.629,13.688H20.082V7.563h10.553v6.126H30.629z";
export const iconKaKao =
  "M12,0C5.373,0,0,4.238,0,9.465c0,3.403,2.277,6.386,5.693,8.054 c-0.251,0.935-0.909,3.387-1.04,3.912c-0.163,0.65,0.239,0.643,0.502,0.467c0.206-0.138,3.288-2.229,4.618-3.13 c0.722,0.106,1.466,0.163,2.227,0.163c6.627,0,12-4.238,12-9.466C24,4.238,18.627,0,12,0";

export const signinAPI = "http://18.219.100.135:8000/user/signin";
export const signupAPI = "http://18.219.100.135:8000/user/signup";
export const shoesListAPI = "http://18.219.100.135:8000/product";
export const shoeDetailAPI = "http://18.219.100.135:8000/product/detail/";
export const orderAPI = "http://18.219.100.135:8000/order";
export const cartAPI = "http://18.219.100.135:8000/order/cart";
export const cartUpdateAPI = "http://18.219.100.135:8000/order/update";
export const cartDeleteAPI = "http://18.219.100.135:8000/order/delete";
export const mainAPI = "http://18.219.100.135:8000/product";
export const instaAPI = "http://18.219.100.135:8000/instagram";

export const magnifier =
  "M19 0C11.8 0 6 5.8 6 13c0 3 1 5.7 2.7 7.9L.8 28.8l2.4 2.4 7.9-7.9C13.3 25 16 26 19 26c7.2 0 13-5.8 13-13S26.2 0 19 0zm0 22.5c-5.3 0-9.5-4.3-9.5-9.5s4.3-9.5 9.5-9.5c5.3 0 9.5 4.3 9.5 9.5s-4.2 9.5-9.5 9.5z";
export const collectionImg =
  "https://www.converse.com/on/demandware.static/-/Library-Sites-SharedLibrary/default/dw0b744b28/firstspirit/media/09_global_elements/2020_fall/search_nav_5/07_35/D-Converse-SearchModule-JULY-20-Converse-CLASSICS~1.jpg";
export const memberOnlyImg =
  "https://www.converse.com/on/demandware.static/-/Library-Sites-SharedLibrary/default/dw8b1211a2/firstspirit/media/09_global_elements/2020_fall/search_nav_5/07_35/D-Converse-SearchModule-JULY-20-Converse-CUSTOM.jpg";
export const jackpurcellImg =
  "https://image.converse.co.kr/cmsstatic/product/164057C_164057C_pdp-primary.jpg?gallery=";
export const chuckImg =
  "https://image.converse.co.kr/cmsstatic/product/168508C_168508C_hover.jpg?browse=";
export const gif =
  "https://image.converse.co.kr/cmsstatic/theme/b-commerce/assets/images/preloader.gif";

export const colorValues = {
  black: "#000000",
  blue: "#0600FF",
  green: "#009900",
  indigo: "#141936",
  purple: "#6600CC",
  brown: "#986633",
  gray: "#999999",
  khaki: "#A39263",
  beige: "#F0E4D2",
  red: "#FF0400",
  orange: "#FF6600",
  pink: "#FFB6C1",
  yellow: "#FFCC00",
  white: "#FFFFFF",
};
